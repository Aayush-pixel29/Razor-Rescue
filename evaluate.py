...
def determine_truth(record):
    ...
    elif "expired" in desc: return "card_expired"
    elif "risk" in desc: return "risky_card"
    return "network_error"

def main():
    parser = argparse.ArgumentParser(description="Razor-Rescue Evaluation Harness")
    parser.add_argument("--records", type=int, default=100, help="Number of records to simulate")
    parser.add_argument("--fast", action="store_true", help="Bypass LLM network call for speed (uses fallback logic)")
    args = parser.parse_args()
    
    print(f"[START] Running Razor-Rescue Evaluation Harness on {args.records} records")
    setup_db()
    
    print("\n[1] Generating synthetic payments with ground truths...")
    raw_records = generate_synthetic_batch(args.records)
    
    classifier = PaymentClassifierAgent()
    decision_engine = DecisionEngine()
    
    # We can force the fallback engine if --fast is passed to simulate 1000s of records quickly
    if args.fast:
        print("Fast mode enabled: using fallback classifier to populate data quickly.")
        classifier_fn = classifier._fallback_classification
    else:
        classifier_fn = classifier.classify_failure

    print("\n[2] Running Strategy A: Dumb Baseline (Blindly retry everything immediately)")
    ...
    for i, raw_record in enumerate(raw_records):
        ...
        if decision.action == "send_payment_link":
            status = "link_sent" # We don't blindly count links as money recovered here
            amount = 0
        elif decision.action == "retry_immediately":
            success, amount, msg, is_bad = PaymentSimulator.simulate_immediate_retry(truth, pr)
            status = "recovered" if success else "failed"
        elif decision.action == "retry_delayed":
            status = "scheduled_24h"
